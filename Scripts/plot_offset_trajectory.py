...
Usage: py plot_offset_trajectory.py [--run-id 1] [--stride 25] [--axis col|row]
         [--window 6] [--centers A B] [--matter-ceiling 1e4] [--db PATH]
"""
import argparse
import sqlite3
...
def peak_centroid(grid, mask, ax, window):
    ...
    return float((idx * sub).sum() / sub.sum() + (rlo if ax == 0 else clo))
...
def main():
    ...
    for n, ts in enumerate(steps):
        ...
        for col, row, gsum, dsum in conn.execute(
            """SELECT cp.col, cp.row, SUM(c.matter_density), SUM(c.rip_dimple)
               ...
               GROUP BY cp.col, cp.row""", (args.run_id, ts)):
        ...
        mx = conn.execute(
            ...
            (args.run_id, ts)).fetchone()[0]
        lg = peak_centroid(gas, lo_mask, ax, args.window)
        ld = peak_centroid(dim, lo_mask, ax, args.window)
        hg = peak_centroid(gas, hi_mask, ax, args.window)
        hd = peak_centroid(dim, hi_mask, ax, args.window)
        T.append(ts)
        lo_g.append(lg); lo_d.append(ld); hi_g.append(hg); hi_d.append(hd)
        lo_off.append((ld - lg) * (+1.0)); hi_off.append((hd - hg) * (-1.0))
        blown.append(mx is not None and mx > args.matter_ceiling)
    ...
    T = np.array(T); blown = np.array(blown)
    def shade(a):
    ...
    fig.suptitle(f"Gas-dimple offset trajectory — Run {args.run_id} "
                 f"(axis={args.axis}, red = blown-up/unreliable)", fontsize=12)
    a1.plot(T, lo_g, "-", color=ORANGE, label="LOW gas")
    a1.plot(T, lo_d, "--", color=ORANGE, label="LOW dimple")
    a1.plot(T, hi_g, "-", color=BLUE, label="HIGH gas")
    a1.plot(T, hi_d, "--", color=BLUE, label="HIGH dimple")
    a1.set_ylabel(f"centroid ({args.axis})"); a1.legend(loc="upper right", fontsize=8)
    shade(a1); a1.grid(True, alpha=0.3)
    sep = np.abs(np.array(hi_g) - np.array(lo_g))
    ...
    a3.plot(T, lo_off, color=ORANGE, label="LOW offset (dimple-lead +)")
    a3.plot(T, hi_off, color=BLUE, label="HIGH offset (dimple-lead +)")
    ...
    plt.tight_layout()
    out = OUTPUT_DIR / f"offset_trajectory_run{args.run_id}.png"
