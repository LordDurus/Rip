...
if %timestep% GTR 249 (
	py offset_diagnostic.py --run-id %run_id% --max-timestep 250 >> %validation_file%
	if %timestep% GTR 1499 (
		py offset_diagnostic.py --run-id %run_id% --max-timestep 1500 >> %validation_file%
	}
	if %timestep% GTR 2999 (
		py offset_diagnostic.py --run-id %run_id% --max-timestep 3000 >> %validation_file%
	}
	if %timestep% GTR 4998 (
		py offset_diagnostic.py --run-id %run_id% --max-timestep 4999 >> %validation_file%
	}
)
